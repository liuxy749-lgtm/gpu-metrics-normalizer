...
| `DCGM_FI_DEV_GPU_UTIL` | `gpu_utilization_ratio` | GPU 利用率，归一化为 0-1 |
| `DCGM_FI_DEV_MEM_COPY_UTIL` | `gpu_memory_usage_ratio` | 显存/内存控制器利用率，归一化为 0-1 |
...
| `ix_gpu_utilization` | `gpu_utilization_ratio` | GPU 利用率，归一化为 0-1 |
...
| `mtgpu_power_usage_milliwatts` | `gpu_power_usage_watts` | 功耗，从 milliwatts 转换为 watts |
| `mtgpu_gpu_utilization` | `gpu_utilization_ratio` | GPU 利用率，归一化为 0-1 |
...
| `bway_power` | `gpu_power_usage_watts` | 当前功耗；仅保留 `wayId="0"` |
| `gpu_usage` | `gpu_utilization_ratio` | GPU 利用率，归一化为 0-1 |
| `memory_total` | `gpu_memory_total_bytes` | 显存总量；仅保留 `memType="vram"` |
| `memory_used` | `gpu_memory_used_bytes` | 显存已用；仅保留 `memType="vram"` |
...
| `dcu_utilizationrate` | `gpu_utilization_ratio` | DCU 利用率，归一化为 0-1 |
...
| `TSINGMICRO_KUIPER_UTILIZATION` | `gpu_utilization_ratio` | GPU 利用率，归一化为 0-1 |
...
| `node_xpu_utilization` | `gpu_utilization_ratio` | XPU 利用率，归一化为 0-1 |
...
| `node_xpu_memutil` | `gpu_memory_usage_ratio` | 显存使用率，归一化为 0-1 |
...
| `enflame_gcu_usage` | `gpu_utilization_ratio` | GCU 利用率，归一化为 0-1 |
| `enflame_gcu_memory_total_bytes` | `gpu_memory_total_bytes` | 显存总量 |
| `enflame_gcu_memory_used_bytes` | `gpu_memory_used_bytes` | 显存已用 |
| `enflame_gcu_memory_usage` | `gpu_memory_usage_ratio` | 显存使用率，归一化为 0-1 |
| `enflame_gcu_health` | `gpu_state` | 健康状态；样例中 `healthmsg="Healthy"` 时值为 `2` |
| `enflame_gcu_clock` | `gpu_clock_mhz` | GCU 核心频率 |
| `enflame_gcu_cluster_usage` | `gpu_cluster_usage_ratio` | Cluster 利用率，归一化为 0-1 |
| `enflame_gcu_sip_usage` | `gpu_sip_usage_ratio` | SIP 利用率，归一化为 0-1 |
| `enflame_gcu_power_capability` | `gpu_power_capability_watts` | 功耗能力/功耗上限 |
| `enflame_gcu_power_usage` | `gpu_power_usage_ratio` | 功耗占比，归一化为 0-1；不要当作 watts 使用 |
...
| `npu_chip_info_ai_core_utilization` | `gpu_utilization_ratio` | AI Core 利用率，归一化为 0-1 |
| `npu_chip_info_hbm_total_memory` | `gpu_memory_total_bytes` | HBM 总量 |
| `npu_chip_info_hbm_used_memory` | `gpu_memory_used_bytes` | HBM 已用 |
| `npu_chip_info_hbm_utilization` | `gpu_memory_usage_ratio` | HBM 使用率，归一化为 0-1 |
...
| `npu_chip_info_hbm_bandwidth_utilization` | `gpu_hbm_bandwidth_ratio` | HBM 带宽利用率，归一化为 0-1 |
| `machine_npu_nums` | `machine_npu_nums` | 机器 NPU 数量，透传辅助指标 |

### 寒武纪 / Cambricon

寒武纪 MLU590 暂不提供 OTel 配置。原因是当前没有可确认的原始 exporter 指标样例，不能把 `mlu_*` 这类泛化写法作为可用配置发布。
