...
依赖: 仅标准库
用法: python3 gpu_info_xiwang.py [--json]
"""
import json
import os
import re
...
def collect():
    ...
    while i < len(lines) - 1:
        ...
        if s1 and s1[0][0].isdigit() and len(s1) >= 2 and len(s2) >= 3:
            ...
            mm = re.search(r"(\S+)\s*/\s*(\d+)MiB", s2[1])
            mu = re.search(r"([-\d.]+)%", s2[2])
            if mt and mp and mm and mu:
                util = float(mu.group(1)); power = float(mp.group(1)); temp = float(mt.group(1))
                ms = mm.group(1).replace("B", "")
                mem_used = 0.0 if not ms else float(ms) / 1048576
                mem_total = float(mm.group(2))
                uuid = ""
                lu = run_cmd("pt_smi -l")
                mu2 = re.search(r"GPU %s:\s*\S+\s+\(UUID:\s*(\S+)\)" % gid, lu)
