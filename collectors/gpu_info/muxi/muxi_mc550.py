...
依赖: 仅标准库
用法: python3 gpu_info_muxi.py [--json]
"""
import json
import os
import re
...
def collect():
    ...
    while i < len(lines) - 1:
        ...
        if s1 and s1[0][0].isdigit() and len(s1) >= 4 and len(s2) >= 3:
            p0 = s1[0].split()
            gid = p0[0]; name = " ".join(p0[1:])
            mu = re.search(r"([-\d.]+)%", s1[3])
            mp = re.search(r"([-\d.]+)W\s*/\s*([-\d.]+)W", s2[0])
            mt = re.search(r"([-\d.]+)C", s2[1])
            mm = re.search(r"([-\d.]+)\s*/\s*([-\d.]+)\s*MiB", s2[2])
            if mu and mp and mt and mm:
                util = float(mu.group(1)); power = float(mp.group(1)); temp = float(mt.group(1))
                mem_used = float(mm.group(1)); mem_total = float(mm.group(2))
                sn = ""
                sq = run_cmd("mx-smi --show-sn -i %s" % gid)
                ms = re.search(r"chip serial\s*:\s*(\S+)", sq)
                if ms: sn = ms.group(1)
