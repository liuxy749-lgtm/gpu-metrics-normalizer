...
注意：这里列出的是“本机脚本解析能力”，不等同于仓库根目录 `configs/` 下的 exporter/OTel 归一化配置清单。某些厂商可能已经有本地脚本，但尚未提供对应的 OTel 配置片段；完整 exporter 归一化支持范围请以根目录 README 和 `docs/vendor-matrix.md` 为准。
