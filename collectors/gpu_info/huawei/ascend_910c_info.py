...
依赖: 仅标准库
用法: python3 gpu_info_ascend.py [--json]
"""
import json
import os
import re
...
def collect():
    text = run_cmd("npu-smi info")
    gpus = []
    lines = text.splitlines()
    i = 0
    while i < len(lines) - 1:
        s1 = segs(lines[i]); s2 = segs(lines[i+1])
        if s1 and s1[0][0].isdigit() and len(s1) >= 3 and len(s2) >= 3:
            p0 = s1[0].split()
            gid = p0[0]; name = " ".join(p0[1:])
            p1 = re.search(r"([-\d.]+)\s+([-\d.]+)", s1[2])
            u1 = re.search(r"([-\d.]+)\s+([-\d.]+)\s*/\s*([-\d.]+)\s+([-\d.]+)\s*/\s*([-\d.]+)", s2[2])
            if p1 and u1:
                power = float(p1.group(1)) if p1.group(1) != "-" else 0.0
                temp = float(p1.group(2))
                util = float(u1.group(1))
                mem_used = float(u1.group(4)); mem_total = float(u1.group(5))
                sn = ""
                b = run_cmd("npu-smi info -t board -i %s" % gid)
                ms = re.search(r"Serial Number\s*:\s*(\S+)", b)
                if ms: sn = ms.group(1)
